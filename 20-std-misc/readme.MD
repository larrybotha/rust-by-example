...
- once `stdin` is piped to, it is dropped, which then results in the command
  actually being called. Without the `stdin` being dropped, the command
  wouldn't get called
...
- the `std::fs` module provides an interface for dealing with the filesystem
...
- `.collect` only cares about what values are being collected _into_ - not what
  the actual values are. e.g. are we collecting into a `Vec` or `
